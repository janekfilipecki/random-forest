    ...
    def fit(
        ...
        for _ in range(self._forest_size):
            ...
            if self._enable_feature_bagging:
                # Secure target feature column from being filtered, add it
                # later after feature bagging
                output_column = tree_train_data[target_feature]
                tree_train_data = tree_train_data.drop(
                    columns=[target_feature]
                )
                tree_train_data = tree_train_data.sample(
                    frac=self._max_features, axis=1, replace=False
                )
                tree_train_data = pd.concat(
                    [tree_train_data, output_column], axis=1
                )
            # Create tree
            tree = CART(
